...
WITH CHECK OPTION;`,
		...
		test("añade la vista EDITORES_CD", { only: false }, async () => {
			const debug = false;
			const result = await qb
...
OR DISQUERAS_CD.ID_DISQUERA = 5402));`,
				...
				assert.equal(
					result.error,
					"[Driver execute] Table 'EDITORES_CD' already exists",
				);
			}
		});
...
WHERE DISCOS_COMPACTOS.ID_DISQUERA = DISQUERAS_CD.ID_DISQUERA;`,
	...
	describe("Roles", () => {
		...
		test("crear un rol", { only: false }, async () => {
			const debug = false;
			const result = await qb
		...
		test("Gestion de roles y privilegios", { only: false }, async () => {
			const debug = false;
		...
		test(
			"otorga el privilegio SELECT en la vista CDS_EN_EXISTENCIA a PERSONAL_VENTAS",
			{ only: false },
			async () => {
				const debug = true;
...
AND TABLE_NAME = 'CDS_EN_EXISTENCIA'`,
				...
				const grant = data.find(
					(item) => item.TABLE_NAME === "cds_en_existencia",
				);
				assert.ok(
				...
				assert.equal(
					await otorga.toString(),
					"USE INVENTARIO;\nGRANT SELECT ON INVENTARIO.CDS_EN_EXISTENCIA TO 'PERSONAL_VENTAS'@'%';",
				);
			},
		);
		test(
			"al rol PERSONAL_VENTAS Se otorgan los privilegios SELECT, INSERT y UPDATE (sobre columna) en la tabla DISCOS_COMPACTOS",
			{ only: false },
			async () => {
...
WHERE  TABLE_SCHEMA = 'inventario'`,
		...
		test(
			"otorga el rol 'PERSONAL_VENTAS' al usurio MRKT@localhost",
			{ only: false },
			async () => {
				const debug = true;
				const otorga = await qb
		...
		test(
			"revocar el privilegio SELECT a PERSONAL_VENTAS de la tabla CDS_EN_EXISTENCIA",
			{ only: false },
			async () => {
				const debug = false;
				const result = await qb
				...
				assert.equal(
					await result.toString(),
					"USE INVENTARIO;\nREVOKE IF EXISTS SELECT ON INVENTARIO.CDS_EN_EXISTENCIA FROM 'PERSONAL_VENTAS'@'%' IGNORE UNKNOWN USER;",
				);
				const data = await getResultFromTest(
					databaseTest,
					`SELECT GRANTEE, TABLE_NAME, PRIVILEGE_TYPE 
...
WHERE  TABLE_SCHEMA = 'inventario'`,
				...
				assert.ok(
					!grants.includes("SELECT"),
					"El privilegio 'SELECT' aun existe",
				);
			},
		);
		test(
...
WHERE  TABLE_SCHEMA = 'inventario'`,
		...
		test(
			"eliminar al rol/usuario 'MRKT' el rol 'PERSONAL_VENTAS'",
			{ only: false },
			async () => {
				const debug = false;
				const result = await qb
		...
		test("eliminar el rol MRKT", { only: false }, async () => {
		...
		test("eliminar el rol PERSONAL_VENTAS", { only: false }, async () => {
			const result = await qb
				.dropRoles(
...
WHERE EN_EXISTENCIA > 9 );`;
		...
		//fin test
		test("uso de SOME", { only: false }, async () => {
...
WHERE EN_EXISTENCIA > 9 );`;
	...
	describe("uso de funciones SET capitulo 10", async () => {
		...
		test("crea tabla CDS_VENDIDOS", { only: false }, async () => {
			const cdsVendidos = {
...
('B.B. King', 'Blues on the Bayou', 18);`;
			...
			assert.ok(
				cdsVendidosRows.every((item) => nombresCdInTable.includes(item[1])),
				"Los 'NOMBRE_CD' no existen en la base de datos",
			);
		});
		//fin test
		test("contar registros", { only: false }, async () => {
...
FROM CDS_VENDIDOS;`;
		...
		//fin test
		test("contar valores en una columna", { only: false }, async () => {
...
WHERE VENDIDOS > 20;`;
			...
			assert.equal(
				values.length,
				rows.map((item) => item.TOTAL_DE_ARTISTAS)[0],
			);
		});
		//fin test
		test("contar valores unicos en una columna", { only: false }, async () => {
...
WHERE VENDIDOS > 20;`;
		...
		test("el valor mas alto para una columna", { only: false }, async () => {
...
FROM CDS_VENDIDOS );`;
		...
		//fin test
		test(
			"el valor 'maximo' para una columna usando agrupacion",
			{ only: false },
			async () => {
...
GROUP BY NOMBRE_ARTISTA;`;
		...
		//fin test
		test(
			"suma de valores de una columna usando 'GROUP BY'",
			{ only: false },
			async () => {
...
GROUP BY NOMBRE_ARTISTA;`;
		...
		//fin test
		test(
			"promedio de valores de una columna usando 'GROUP BY'",
			{ only: false },
			async () => {
					...
					.reduce((acumulados, actual) => {
						if (acumulados[actual.NOMBRE_ARTISTA] === undefined) {
							...
							acumulados[actual.NOMBRE_ARTISTA] = {
								...
								avg:
									(acumulados[actual.NOMBRE_ARTISTA].acumulado +
										actual.VENDIDOS) /
